...
视频拉片工具是一个 AI 驱动的视频镜头分析应用。它可以上传视频、自动检测镜头边界、切割镜头片段、调用通义千问多模态模型生成镜头语言分析，并导出 Excel/PDF 报告。

> 当前项目更适合作为本地工具、研究原型或二次开发基座。若要公网部署，请先处理 `AUDIT.md` 中列出的访问控制、密钥、上传限制等安全问题。
...
- 用户与积分：支持邮箱密码登录、微信扫码登录、积分扣减、管理员重置积分。
...
这个仓库已经补充了 MIT License 和中英文 README，但在真正公开部署前建议至少完成以下事项：

- 所有视频读写、检测、分析、导出接口现在都要求登录并校验资源归属。
- 生产环境会拒绝默认或过短的 `SECRET_KEY`。
- 上传文件会使用 UUID 存储名并限制大小；公网部署时仍建议在反向代理层设置请求体上限。
