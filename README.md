...
默认模板含 Qwen ASR 字段，因此真正启动分析前必须配置阿里云可访问的 `PUBLIC_VIDEO_BASE_URL`；未配置时后端会明确拒绝任务，不会清空旧结果。
...
| `QWEN_VIDEO_INPUT_MODE` | `auto` | Qwen 视频输入模式：`auto` 小文件 Base64、大文件签名 URL；也可强制 `base64` 或 `url`。 |
| `VISION_MODEL_NAME` | `qwen3.7-plus` | 画面、镜头、段落和运营分析模型。 |
| `ASR_MODEL_NAME` | `qwen3-asr-flash-filetrans` | 全片台词与时间戳模型。 |
| `ASR_API_BASE_URL` | 新加坡异步接口 | 必须与 API Key 地域一致。 |
...
默认配置在 `backend/prompt_configs/default.json`。登录后可在每个视频的“分析配置”中可视化调整，并保存为私有 preset：
...
字段支持嵌套结构和数组类型。每次分析会保存配置快照，后续修改草稿不会改变历史报告；应用新配置需要完整重新分析。

数据库升级说明：本版新增 `analysis_presets`、`video_analysis_configs`、`video_transcripts` 和 `analysis_task_snapshots` 表。生产环境请在升级前备份数据库，并执行 `cd backend && alembic upgrade head`；本地开发启动仍会通过 SQLAlchemy 创建缺失表。旧分析结果保持旧格式，只有完整重新分析才会切换到 v2 schema。
